...
exports.performance = function(USER_ID, ROLE) {
    if (ROLE === 'block') {
        return (
            ...
            "SELECT a.block_code, a.block_name, a.panchayat_code, a.panchayat_name, b.tot_trn, b.year, b.month, b.mrc_mre, b.mre_wlg, b.wlg_wls, b.wls_fto, b.fto_sn1, b.sn1_sn2, b.sn2_prc FROM (SELECT block_code, block_name, panchayat_code, panchayat_name FROM panchayats WHERE block_code IN (SELECT region_code FROM user_regions WHERE user_id='" + USER_ID + "')) a LEFT JOIN (SELECT block_code, panchayat_code, SUM(total_transactions) AS tot_trn,YEAR(date) AS year, MONTH(date) AS month,ROUND(sum(mrc_mre_mean*total_transactions)/sum(total_transactions),0) mrc_mre,ROUND(sum(mre_wlg_mean*total_transactions)/sum(total_transactions),0) mre_wlg,ROUND(sum(wlg_wls_mean*total_transactions)/sum(total_transactions),0) wlg_wls,ROUND(sum(wls_fto_mean*total_transactions)/sum(total_transactions),0) wls_fto,ROUND(sum(fto_firstsign_mean*total_transactions)/sum(total_transactions),0) fto_sn1,ROUND(sum(firstsign_secondsign_mean*total_transactions)/sum(total_transactions),0) sn1_sn2,ROUND(sum(secondsign_processed_mean*total_transactions)/sum(total_transactions),0) sn2_prc FROM panchayat_delays_duration WHERE gender = 'both' AND bank_type = 'all' AND date_type = 'processed_date' AND total_transactions > 0 AND panchayat_code <> '0000000000' AND block_code IN (SELECT region_code FROM user_regions WHERE user_id='" +
            USER_ID +
            "') GROUP BY YEAR(date), MONTH(date), panchayat_code) b ON a.block_code=b.block_code AND a.panchayat_code=b.panchayat_code ORDER BY a.block_code, a.panchayat_code, b.year, b.month;"
        );
    } else if (ROLE === 'district') {
        return (
            ...
            "SELECT a.district_code, a.district_name, a.block_code, a.block_name, b.tot_trn, b.year, b.month, b.mrc_mre, b.mre_wlg, b.wlg_wls, b.wls_fto, b.fto_sn1, b.sn1_sn2, b.sn2_prc FROM (SELECT district_code, district_name, block_code, block_name FROM blocks WHERE district_code IN (SELECT region_code FROM user_regions WHERE user_id='" + USER_ID + "')) a LEFT JOIN (SELECT district_code, block_code, SUM(total_transactions) AS tot_trn,YEAR(date) AS year, MONTH(date) AS month,ROUND(sum(mrc_mre_mean*total_transactions)/sum(total_transactions),0) mrc_mre,ROUND(sum(mre_wlg_mean*total_transactions)/sum(total_transactions),0) mre_wlg,ROUND(sum(wlg_wls_mean*total_transactions)/sum(total_transactions),0) wlg_wls,ROUND(sum(wls_fto_mean*total_transactions)/sum(total_transactions),0) wls_fto,ROUND(sum(fto_firstsign_mean*total_transactions)/sum(total_transactions),0) fto_sn1,ROUND(sum(firstsign_secondsign_mean*total_transactions)/sum(total_transactions),0) sn1_sn2,ROUND(sum(secondsign_processed_mean*total_transactions)/sum(total_transactions),0) sn2_prc FROM block_delays_duration WHERE gender = 'both' AND bank_type = 'all' AND date_type = 'processed_date' AND total_transactions > 0 AND district_code IN (SELECT region_code FROM user_regions WHERE user_id='" +
            USER_ID +
            "') GROUP BY YEAR(date), MONTH(date), block_code) b ON a.district_code=b.district_code AND a.block_code=b.block_code ORDER BY a.district_code, a.block_code, b.year, b.month;"
        );
    }
};
    ...
    if (VERSION === 1) {
        ...
        if (ROLE === 'block') {
            return (
                ...
                USER_ID +
                "') GROUP BY YEAR(date), MONTH(date), block_code) b ON a.block_code=b.block_code ORDER BY a.block_code, b.year, b.month;" +
                "SELECT a.block_code, a.panchayat_code, a.panchayat_name, b.tot_trn, b.year, b.month, b.mrc_mre, b.mre_wlg, b.wlg_wls, b.wls_fto, b.fto_sn1, b.sn1_sn2, b.sn2_prc, b.mrc_prc from (SELECT block_code, panchayat_code, panchayat_name FROM panchayats WHERE block_code IN (SELECT region_code FROM user_regions WHERE user_id='" + USER_ID + "')) a LEFT JOIN (SELECT block_code, panchayat_code, SUM(total_transactions) AS tot_trn,YEAR(date) AS year, MONTH(date) AS month,ROUND(sum(mrc_mre_mean*total_transactions)/sum(total_transactions),1) mrc_mre,ROUND(sum(mre_wlg_mean*total_transactions)/sum(total_transactions),1) mre_wlg,ROUND(sum(wlg_wls_mean*total_transactions)/sum(total_transactions),1) wlg_wls,ROUND(sum(wls_fto_mean*total_transactions)/sum(total_transactions),1) wls_fto,ROUND(sum(fto_firstsign_mean*total_transactions)/sum(total_transactions),1) fto_sn1,ROUND(sum(firstsign_secondsign_mean*total_transactions)/sum(total_transactions),1) sn1_sn2,ROUND(sum(secondsign_processed_mean*total_transactions)/sum(total_transactions),1) sn2_prc, ROUND(sum(mrc_processed_mean*total_transactions)/sum(total_transactions),1) mrc_prc FROM panchayat_delays_duration WHERE gender = 'both' AND bank_type = 'all' AND date_type = 'processed_date' AND total_transactions > 0 AND panchayat_code <> '0000000000' AND block_code IN (SELECT region_code FROM user_regions WHERE user_id='" +
                USER_ID +
                "') GROUP BY YEAR(date), MONTH(date), panchayat_code) b ON a.block_code=b.block_code AND a.panchayat_code = b.panchayat_code ORDER by a.block_code, a.panchayat_code, b.year, b.month;" +
                "SELECT state_code FROM blocks WHERE block_code IN (SELECT region_code FROM user_regions WHERE user_id='" +
                USER_ID +
            ...
            return (
                ...
                USER_ID +
                "') GROUP BY YEAR(date), MONTH(date), district_code) b ON a.district_code=b.district_code ORDER BY a.district_code, b.year, b.month;" +
                "SELECT a.district_code, a.block_code, a.block_name, b.tot_trn, b.year, b.month, b.mrc_mre, b.mre_wlg, b.wlg_wls, b.wls_fto, b.fto_sn1, b.sn1_sn2, b.sn2_prc, b.mrc_prc from (SELECT district_code, block_code, block_name FROM blocks WHERE district_code IN (SELECT region_code FROM user_regions WHERE user_id='" + USER_ID + "')) a LEFT JOIN (SELECT district_code, block_code, SUM(total_transactions) AS tot_trn,YEAR(date) AS year, MONTH(date) AS month,ROUND(sum(mrc_mre_mean*total_transactions)/sum(total_transactions),1) mrc_mre,ROUND(sum(mre_wlg_mean*total_transactions)/sum(total_transactions),1) mre_wlg,ROUND(sum(wlg_wls_mean*total_transactions)/sum(total_transactions),1) wlg_wls,ROUND(sum(wls_fto_mean*total_transactions)/sum(total_transactions),1) wls_fto,ROUND(sum(fto_firstsign_mean*total_transactions)/sum(total_transactions),1) fto_sn1,ROUND(sum(firstsign_secondsign_mean*total_transactions)/sum(total_transactions),1) sn1_sn2,ROUND(sum(secondsign_processed_mean*total_transactions)/sum(total_transactions),1) sn2_prc, ROUND(sum(mrc_processed_mean*total_transactions)/sum(total_transactions),1) mrc_prc FROM block_delays_duration WHERE gender = 'both' AND bank_type = 'all' AND date_type = 'processed_date' AND total_transactions > 0 AND district_code IN (SELECT region_code FROM user_regions WHERE user_id='" +
                USER_ID +
                "') GROUP BY YEAR(date), MONTH(date), block_code) b ON a.district_code=b.district_code AND a.block_code = b.block_code ORDER by a.district_code, a.block_code, b.year, b.month;" +
