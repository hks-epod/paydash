...
exports.data = {
    ...
    handler: function(request, reply) {
        ...
        var queryTemplates = {
            ...
            'users_1session_7days':'SELECT a.session_flag_count/a.user_count AS value, date'+compTextAs+' FROM (SELECT SUM(session_flag) as session_flag_count, COUNT(DISTINCT user_id) as user_count, date'+compText+' FROM (SELECT IF(rolling_session_count>0,1,0) as session_flag, date, user_id'+compText+' FROM ga_sessions '+whereClause+'GROUP BY user_id,date'+compText+') b GROUP BY date'+compText+') a;',
