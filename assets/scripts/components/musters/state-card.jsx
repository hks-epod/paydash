...
const StateCard = React.createClass({
    toggleModal() {
        const state = this.state.modalOpen;
        this.setState({ modalOpen: !state });
        ga('send', 'event', {
            eventCategory: 'State Muster detail',
            eventAction: 'click',
            eventLabel: this.props.data.district_code + '/' + this.props.identifier
        });
    },
            ...
            <div className="pure-u pure-u-6-24">
                <div className="card">
                    <div className="state-card-top">
                        <div className="card-head">{this.props.data.district_name}</div>
                    </div>
                    <div className="state-card-numbers pure-g">
