...
import StateCard from './state-card.jsx';
import Sort from './sort.jsx';

const DistrictGroup = React.createClass({
    filterCards: function(event) {
        var updatedList = this.props.data.cards;
        updatedList = updatedList.filter(function(item) {
            var match = false;

            if (item.district_name.toLowerCase().search(event.target.value.toLowerCase()) !== -1) {
                match = true;
            }
    ...
    render: function() {
        var _this = this;
        var sortList = ['district_name', 'current_total', 'delayed_total', 'days_to_payment'];
