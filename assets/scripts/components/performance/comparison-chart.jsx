...
const ComparisonChart = React.createClass({
    ...
    loadChart: function() {
        ...
        _this.state.active_lines.forEach(function(comparison_line, index) {
            var region = Region.find(
                _this.props.config.role,
                _this.props.activeRegion,
                _this.props.performance,
                comparison_line
            );
            labels.push(
                region[comparison_line + '_name'] +
                    ' ' +
                    _this.props.translation.comparison.labels[comparison_line]
            );
            var line_data = Parser.lines({
                data: region.data,
                col:
                    _this.state.active_step === '8'
                        ? [1, 2, 3, 4, 5, 6, 7]
                        : [_this.state.active_step],
                isCumulative: _this.state.active_step === '8' ? true : false
            });

            // Workaroud to get just all steps
            if (_this.state.active_step === '8') {
                c_data.push(line_data[6]);
                return;
            }
            if (line_data[0]) {
        ...
        MG.data_graphic({
            ...
            mouseover: function(d, i) {
                ...
                d.values.forEach(function(val, index) {
                    ...
                    var l_span = D3.select(
                        legend_target + ' .mg-line' + val.line_id + '-legend-color'
                    );
                    l_span.text(' ');
                    l_span.text('— ' + labels[val.line_id - 1] + ' : ' + val.value);
                    var format = D3.timeFormat('%b %Y');
                    D3.select('#region_comparison_total_trans').text(
                        _this.props.translation.comparison.total_trans[
                            _this.props.activeRegion.region_type
                        ] +
                            ' ' +
                            format(val.date) +
                            ': ' +
    ...
    componentWillReceiveProps: function(nextProps) {
        ...
        if (
            nextProps.config.role === 'block' &&
            nextProps.activeRegion.region_type === 'panchayat'
        ) {
            comparison_lines = ['state', 'district', 'block', 'panchayat'];
        }
        if (
            nextProps.config.role === 'district' &&
            nextProps.activeRegion.region_type === 'block'
        ) {
            ...
            <div className="pure-g">
                <div className="pure-u-6-24">
                    ...
                    <p>
                        ...
                        <span
                            title={
                                this.props.translation && this.props.translation.comparison.tooltip
                            }
                    ...
                    <select
                        ...
                        className="button button--pc"
                        name=""
                        id="modify-step-controls"
                    >
                        <option value="1">
                            {this.props.translation && this.props.translation.overview.labels[0]}
                        </option>
                        <option value="2">
                            {this.props.translation && this.props.translation.overview.labels[1]}
                        </option>
                        <option value="3">
                            {this.props.translation && this.props.translation.overview.labels[2]}
                        </option>
                        <option value="4">
                            {this.props.translation && this.props.translation.overview.labels[3]}
                        </option>
                        <option value="5">
                            {this.props.translation && this.props.translation.overview.labels[4]}
                        </option>
                        <option value="6">
                            {this.props.translation && this.props.translation.overview.labels[5]}
                        </option>
                        <option value="7">
                            {this.props.translation && this.props.translation.overview.labels[6]}
                        </option>
                        <option value="8">
                            {this.props.translation && this.props.translation.overview.labels[7]}
                        </option>
                    </select>
                    <div id="compareRegion" className="group-selector">
                        {this.state.comparison_lines.map(function(line, i) {
                            ...
                            var label =
                                region[line + '_name'] +
                                ' ' +
                                _this.props.translation.comparison.labels[line];
                                ...
                                <div key={line}>
                                    <label htmlFor={'option-' + line} className="pure-checkbox">
                                        <input
                                            ...
                                            onChange={_this.lineChange}
                                            type="checkbox"
                                            value={line}
                                            data-region={region[line + '_code']}
                                            checked={selected}
                                        />{' '}
                                        {label}
                                    </label>
                    ...
                    <div className="legend comparison_legend" />
                    <div>
                        <span id="region_comparison_total_trans" />
                    </div>
                </div>
